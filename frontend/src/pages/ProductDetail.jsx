...
<div className='lg:w-[20%] md:w-[80%] h-[10%] lg:h-[80%] flex items-center justify-center gap-[50px] lg:gap-[20px] lg:flex-col flex-wrap'>
  {image1 && image1.trim() !== ""  && (
    <div className='md:w-[100px] w-[50px] h-[50px] md:h-[110px] bg-slate-300 border border-gray-300 rounded-md'>
      <img
        src={image1}
        alt=""
  ...
  {/* Main Image */}
   <div className='lg:w-[60%] w-[80%] lg:h-[78%] h-[70%] border-[1px] border-[#80808049] rounded-md  overflow-hidden'>
     ...
     <div  className='lg:w-[50vw] w-[100vw] lg:h-[75vh] h-[40vh] lg:mt-[80px] flex items-start justify-start flex-col py-[20px] px-[30px] md:pb-[20px] md:pl-[20px] lg:pl-[0px] lg:px-[0px] lg:py-[0px] gap-[10px]'>
      ...
      <h1 className='text-[40px] font-semibold text-[aliceblue]'>{productData.name}</h1>
  ...
  productData?.sizes?.map((item, index) => (
    <button key={index} className={`border py-2 px-4 bg-slate-300 rounded-md 
      ${item === size ? 'bg-black text-[#2f97f1] text-[20px]' : ''}`} onClick={() => setSize(item)} >
      {item}
    </button>
  ))
}
          </div>
          <button className='text-[16px] active:bg-slate-500 cursor-pointer bg-[#495b61c9] py-[10px] px-[20px] rounded-2xl mt-[10px] border-[1px] border-[#80808049] text-white shadow-md shadow-black' onClick={()=>addToCart(productData._id,size)}  >Add To Card</button>
                </div>
