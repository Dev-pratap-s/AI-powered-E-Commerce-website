...
const handleLogout = async () => {
  try {
   ...
   console.log("🔍 Logout full response:", res.data);
  ...
  return (
    <div className='w-[100vw] h-[70px] bg-[#ecfafaec] z-10 fixed top-0 flex items-center justify-between px-[30px] shadow-md shadow-black'>
      ...
      {/* Right - Icons */}
      <div className='w-[20%] lg:w-[20%] flex items-center justify-end gap-[20px] relative'>
        ...
        {/* ✅ Show user's first letter when logged in, else Login icon */}

{/* /////// */}



 {!userData && <CgProfile className='w-[29px] h-[29px] text-[#000000]  cursor-pointer' onClick={()=>setShowProfile(prev=>!prev)}/>}
        ...
        <MdOutlineShoppingCart onClick={()=>navigate('/cart')} className='w-[30px] h-[30px] text-black cursor-pointer hidden md:block' />
        <p className='absolute w-[18px] h-[18px] flex items-center justify-center bg-black px-[5px] py-[2px] text-white rounded-full text-[9px] top-[10px] right-[23px] hidden md:flex'>{getCartCount()}</p>
      </div>
      ...
      {showProfile &&
        <div className='absolute w-[220px] h-[150px] bg-[#000000d7] top-[110%] right-[4%] border-[1px] border-[#aaa9a9] rounded-[10px] z-10'>
          <ul className='w-[100%] h-[100%] flex items-start justify-around flex-col text-[17px] py-[10px] text-white'>
            <li className='w-[100%] hover:bg-[#2f2f2f] px-[15px] py-[10px] cursor-pointer' onClick={handleLogout}>LogOut</li>
   ...
   {/* //nache wALI */}
      <div className='w-[100vw] h-[90px] flex items-center justify-between px-[20px] text-[12px]
         fixed bottom-0 left-0 bg-[#191818]   md:hidden'>
             <button className='text-[white] flex items-center justify-center flex-col gap-[2px]' onClick={()=>navigate("/")}><TiHome  className='w-[28px] h-[28px] text-[white] md:hidden'/> Home</button>
             <button className='text-[white] flex items-center justify-center flex-col gap-[2px]' onClick={()=>navigate("collection")}><HiOutlineCollection  className='w-[28px] h-[28px] text-[white] md:hidden'/> Collections</button>
              <button className='text-[white] flex items-center justify-center flex-col gap-[2px] ' onClick={()=>navigate("/contact")}><MdContacts 
 className='w-[28px] h-[28px] text-[white] md:hidden'/>Contact</button>
